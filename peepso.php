...
class PeepSo
{
    ...
    private function __construct()
    {
        ...
        add_action('init', function(){
            ...
            if(strlen(get_transient(PeepSoAdmin::TRANS_CONNECT_TIMEOUT))) {
                return;
            }
            ...
            if (empty($check_plugins)) {
                $response = wp_remote_get('https://www.peepso.com/products.json?version='.PeepSo::PLUGIN_VERSION.'&locale='.get_locale().'&hide_bundle_offer='.PeepSo::get_option('bundle', 0), array('timeout' => 15));

                if (is_array($response)) {
                    delete_transient(PeepSoAdmin::TRANS_CONNECT_TIMEOUT);
                    delete_transient(PeepSoAdmin::TRANS_CONNECT_TIMEOUT_COUNT);
                ...
                if ( is_wp_error( $response ) ) {
                    $failures = (int) get_transient(PeepSoAdmin::TRANS_CONNECT_TIMEOUT_COUNT);
                    $failures++;
                    set_transient(PeepSoAdmin::TRANS_CONNECT_TIMEOUT, print_r($response, TRUE), 300);
                    set_transient(PeepSoAdmin::TRANS_CONNECT_TIMEOUT_COUNT, $failures, 3000);
                }
            }
        ...
        add_action( 'user_register', function( $user_id ) {
            ...
            if (1 === $disable_registration) {
                ...
                // PeepSo#3756 update user_notification if register from third party
        ...
        add_action('wp_ajax_peepso_user_unsubscribe_onsite', function(){
            ...
            if (class_exists('PeepSoGroupsPlugin')) {
                global $wpdb;
                $count = $wpdb->query("UPDATE `{$wpdb->prefix}peepso_group_followers` SET `gf_notify`=0 WHERE `gf_user_id`=$user_id");
            }
        ...
        add_action('wp_ajax_peepso_user_subscribe_emails', function(){
            ...
            if($user_id) {
                ...
                if (class_exists('PeepSoGroupsPlugin')) {
                    global $wpdb;
                    $count = $wpdb->query("UPDATE `{$wpdb->prefix}peepso_group_followers` SET `gf_email`=1 WHERE `gf_user_id`=$user_id");
                }
        ...
        add_action('wp_ajax_peepso_user_unsubscribe_emails', function() {
            ...
            if(PeepSo::is_admin() || $user_id == get_current_user_id()) {
                ...
                if (class_exists('PeepSoEmailDigest')) {
                    if (1 == get_user_meta($user_id, 'peepso_email_digest_receive_enabled', TRUE)) {
                        update_user_meta($user_id, 'peepso_email_digest_receive_enabled', 0);
                        $debug['digest'] = __('Disabled Email Digest subscription', 'peepso-core');
                    } else {
                        $debug['digest'] = __('Email Digest was already disabled', 'peepso-core');
                    }
                }
            }
        ...
        if (PeepSo::get_option('social_login_integration_enable', FALSE)) {
            add_action('twistpress_social_login_after_register', function($user_id, $profile) {
                // get mapping field
                $fields = get_option('peepso_social_field_mapping', array());
                ...
                if (!empty($profile->photoURL) && PeepSo::get_option('social_login_import_avatar', 1)) {
                    $avatar = file_get_contents($profile->photoURL);
		            $avatar_hash = substr(md5(time()), 0, 10);
                    $path = self::get_peepso_dir() . '/users/' . $user_id;
                    @mkdir($path);
            ...
            add_filter('twistpress_social_login_redirect', function($url) {
        ...
        add_action('peepso_action_render_login_form_after', array($this, 'social_login_form'));
        add_action('peepso_after_registration_form', array($this, 'social_login_form'));
...
//		}
    ...
    public function enqueue_scripts_data()
    {
        ...
        $data = apply_filters('peepso_data', array(
            ...
            'external_link_whitelist' => PeepSo::get_option('external_link_whitelist', ''),
...
// @todo HTML rendering methods should probably be refactored
    ...
    {
        ?>
        <h3><?php _e('PeepSo E-mail Notifications', 'peepso-core');?></h3>
        <table class="form-table">
            <tbody>
            <tr>
                <th scope="row"><?php _e('E-mail preferences', 'peepso-core'); ?></th>
                <td id="ps-js-unsub-email">
                    <label for="peepso_unsub_email_notification"><button name="peepso_unsub_email_notification" id="peepso_unsub_email_notification" class="button"><?php _e('Unsubscribe this user from all e-mail notifications', 'peepso-core');?></button></label>
...
//alvar
        if($target_post = json_decode($content)) {
            // varible extra
            $post_ext = get_post($target_post->post_id); 
            
            $my_slug = $post_ext->post_name;
            
            if (is_numeric(substr($my_slug, -1, 1))){
                $my_slug = substr($my_slug,0,-2);
            };
            //$content = get_permalink($target_post->post_id);
            $content = 'http://waterspot.org/listing/'.$my_slug.'';
                ...
                if ($matches) {
                    ...
                    if(count($hashtags)) {
                        ...
                        foreach($hashtags as $hashtag) {
                            ...
                            $hashtag=strip_tags(trim(strtolower($hashtag),'#'));
...
// Profiles
    ...
    public function social_login_form() {
        ...
        if ($social_login_enabled && PeepSo::social_login_enabled()) {
            TwistPress_Social_Login::get_instance()->login_form(TRUE);
        }
    }
